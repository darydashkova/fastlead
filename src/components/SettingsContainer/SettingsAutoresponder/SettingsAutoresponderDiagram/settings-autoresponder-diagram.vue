...
<script>
    import SettingsAutoresponderCard from '../SettingsAutoresponderCard'
    ...
    window.CustomUserObjectConnector = function () {
        this.isConnect = true;
        this.clone = function () {
            return mxUtils.clone(this);
        };
    ...
    export default {
        ...
        methods: {
            ...
            changeObjectValues(newCellValue) {
                ...
                try {
                    let links = newCellValue.messageSettings.linksOutput;

                    links.forEach((link, index) => {
                        if (this.currentCell.children[index+1]) {
                            this.currentCell.children[index+1].value.name = link;
                        } else {
                            let connector = new window.CustomUserObjectConnectorWithLabel(link);
                            let connectors = new mxCell(connector, new mxGeometry(0, 0, 123, 19), 'fillColor=var(--hover-folder-bg)');
                            ...
                            cn.style = 'movable=0'
                            editor.graph.addCell(cn, this.currentCell)
                        }
                    })
                } finally {
                    model.endUpdate();
            ...
            addSidebarIcon(graph, sidebar, prototype, connectors) {
                ...
                d.addEventListener('click', () => {
                    ...
                    try {
                        ...
                        connector.geometry.x = 0;
                        connector.geometry.y = 115;
                        graph.addCell(connector, v1);
            ...
            addStartElement(graph, sidebar, prototype, connectors) {
                let parent = graph.getDefaultParent();
                let model = graph.getModel();
                ...
                if (!mxClient.isBrowserSupported()) {
                    ...
                    editor.graph.getLabel = function (cell) {
                        if (cell && this.isHtmlLabel(cell) && cell.value) {
                            ...
                            let label =
                                cell.value.isStart?
                                `<div class="settings-autoresponder-start-card">
                                    ...
                                    :
                                    cell.value.isConnect?
                                        `<div class="settings-autoresponder-card__link"></div>`
                                    :
                                        cell.value.isConnectWithLabel?
                                        `<div class="settings-autoresponder-card__link-name"><span>${cell.value.name}</span><div class="settings-autoresponder-card__link"></div></div>`
                                        :
                                    `<div class="settings-autoresponder-card">
                                        ...
                                        <div class="settings-autoresponder-card__container">
                                            <textarea class="settings-autoresponder-card__message" onclick="focus"></textarea>
                                        </div>
                                    </div>
                                    `
                    ...
                    this.addStartElement(editor.graph, sidebar, object, connectors);


                    // Adds sidebar icon for the propertie object
                    //Добавить свои пропы в элемент

                    let customObject = new window.CustomUserObject({choice: null, linksOutput: ['Первый', 'Второй']});

                    object = new mxCell(customObject, new mxGeometry(0, 0, 345, 207), '');
                    object.setVertex(true);
                    object.setConnectable(false);

                    this.addSidebarIcon(editor.graph, sidebar, object, connectors);
                }
            },
